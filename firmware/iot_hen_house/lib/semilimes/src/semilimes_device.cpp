...
String semilimes_device::SetOut(String name, int id, int group,	int subgroup, int output_n, int value)
{
	...
	{
		if(name==dev_name || (name=="none" && id==dev_id && group==0 && subgroup==0) || (id==dev_id && group==dev_group && subgroup==0) || (id==dev_id && group==dev_group && subgroup==dev_subgroup))
		{
...
String semilimes_device::GetIn(String name, int id, int group, int subgroup, int input_n)
{
	if(input_n<=dev_n_in)
	{
		if(name==dev_name || (name=="none" && id==0 && group==0 && subgroup==0) || (id==dev_id && group==0 && subgroup==0) 
		|| (id==0 && group==dev_group && subgroup==0) || (id==dev_id && group==dev_group && subgroup==0) 
		|| (id==dev_id && group==dev_group && subgroup==dev_subgroup))
		{
...
String semilimes_device::help()
{
	...
	<p>i, g, s with '0' means all ip, all group, all subg</p>\
	<p>If you dont't set i, g, s in the msg, are '0' by default</p>\
	<p>sample message to set a digital output:</p>\
	<p># > nO_Device1. i1. g1. s1. p0. v1.</p>\
	<p>sample message to get a digital input:</p>\
	<p># < nI_Device1. i1. g1. s1. p1.</p>\
...
String semilimes_device::parse_msg(String msg, String ReceiverId)
{
	...
	if(msg[i]=='#')
	{
		...
		while(i<msg.length())
		{
			...
			if(msg.charAt(i)=='h')//help
			{
				option = msg.charAt(i);
			}			
			if(msg.charAt(i)=='>' || msg.charAt(i)=='<')//>output , <input
			{
			...
			if(msg.charAt(i)=='n')//name
			{
				int j=0;
				i++;
				while(j<(msg.length()-i) && msg.charAt(i+j) != '.')
				{
					name += msg.charAt(i+j);
					j++;
				}
